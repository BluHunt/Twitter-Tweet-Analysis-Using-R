...
library(tmap)
library(wordcloud)
api_key <- "B0aSye4VVVQQsv5gB5y38rq4t"
api_secret_key <- "tGSzjf0VLVU6Wb3gpCo8hJPvNNK2TRcHsHQhPVMq8258eP5Dwd"
access_token <- "1497064956046745600-kGbeX5BoUn5lCwJwlKqyzMwu318YpB"
access_token_secret <- "4b1ViXOVBea2VfI2VTVprgG9Irk1OHe69LpaNSByJNSEX"
twitter_token <- create_token(app = "R Project Twitter",consumer_key = api_key,consumer_secret = api_secret_key,access_token = access_token,access_secret = access_token_secret,set_renv = TRUE)
climate <- search_tweets("climate", n=4000, include_rts=FALSE, lang="en")
Gates_tweets <- get_timeline("@BillGates", n= 2000)
# Remove retweets
...
data <- data.frame(category=c("Organic", "Retweets", "Replies"),count=c(2856, 192, 120))
...
data <- data.frame(category=Gates_app$source,count=Gates_app$count)
data$fraction = data$count / sum(data$count)
data$percentage = data$count / sum(data$count) * 100
data$ymax = cumsum(data$fraction)
data$ymin = c(0, head(data$ymax, n=-1))
...
xlab(NULL) +
coord_flip() +
labs(y = "Count",x = "Unique words",title = "Most frequent words found in the tweets of Bill Gates",
...
set.seed(1234)
wordcloud(Gates_retweets$retweet_screen_name, min.freq=3, scale=c(2, .4), random.order=FALSE, rot.per=0.20, 
          colors=brewer.pal(8, "Dark2"))
